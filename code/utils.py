...
# Psuedorandomization
random_set = pd.read_csv(
    task_directory + '/code/pseudorandom_set1.csv')['value'].to_numpy()/100
...
def order_blocks(num_of_blocks):
    ...
    return int(num_of_blocks/2) * [False, True]
...
def display_fixation():
    gaze_target = pygame.image.load(
        task_directory + "/media/images/misc/plus_symbol.png").convert()
    gaze_target = pygame.transform.smoothscale(gaze_target, (25, 25))
...
def calculate_pseudorandom_outcome(random_set_index, threshold):
    val = random_set[random_set_index]
    random_set_index = random_set_index + 1
    if val <= threshold:
        return 0, random_set_index
    else:
        return 1, random_set_index


def generate_trial_points(block_type, shape, is_practice_trial, random_set_index):
    if is_practice_trial:
        return 10, random_set_index  # If practice Trial give Postive reward
    trial_reward_prob = reward_conflict_prob[block_type][shape][0]
    which_sign, random_set_index = calculate_pseudorandom_outcome(
...
def display_stimulus(screen, block, shape, subject, image_file_names, is_practice_trial, random_set_index):
    trial_prov_prob = reward_conflict_prob[block][shape][1]
    image_type = np.random.choice(
        [0, 1], p=[trial_prov_prob, 1-trial_prov_prob])
    image_type, random_set_index = calculate_pseudorandom_outcome(
        random_set_index, trial_prov_prob)
    if is_practice_trial:
    ...
    return image_path, random_set_index
...
def display_left_arrow_key():
    left_arrow_target = pygame.image.load(
        task_directory + "/media/images/misc/left_arrow.png").convert()
    left_arrow_target = pygame.transform.smoothscale_by(
        left_arrow_target, 0.25)
    left_arrow_rect = left_arrow_target.get_rect()
    left_arrow_coords = np.array(
        size) - np.array([left_arrow_rect.w/2+size[0]/2+size[0]/30, left_arrow_rect.h + size[1]/36])
    ...
    # -1.
    screen.blit(text_surf_l, (left_arrow_coords[0]-text_surf_l.get_width(
    )-size[0]/200, left_arrow_coords[1] + size[1]/200))


def display_right_arrow_key():
    right_arrow_target = pygame.image.load(
        task_directory + "/media/images/misc/right_arrow.png").convert()
    right_arrow_target = pygame.transform.smoothscale_by(
        right_arrow_target, 0.25)
    right_arrow_rect = right_arrow_target.get_rect()
    right_arrow_coords = np.array(
        size) - np.array([right_arrow_rect.w/2+size[0]/2-size[0]/30, right_arrow_rect.h + size[1]/36])
    screen.blit(right_arrow_target, right_arrow_coords)
    my_font = pygame.font.SysFont("Arial", int(size[0]/80))
    text_surf_r = my_font.render("Next", True, WHITE)
    display_photodiode_border()
    screen.blit(text_surf_r, (right_arrow_coords[0]+right_arrow_target.get_width(
    )+size[0]/200, right_arrow_coords[1] + size[1]/200))
...
def display_id_query(user_text_subj_id, user_text_study_id, toggle):
    ...
    screen.blit(txtsurf_subj, (size[0]/2 - txtsurf_subj.get_width() /
                2-size[0]/256, size[1]/2 - txtsurf_subj.get_height()/2))
    # Display "Study ID"
    screen.blit(txtsurf_study, (size[0]/2 - txtsurf_subj.get_width()/2-size[0] /
                256, size[1]/2 - txtsurf_subj.get_height()/2+txtsurf_subj.get_height()))

    # render at position stated in arguments
    screen.blit(subj_text_surface, ((
        size[0]+txtsurf_subj.get_width())/2-size[0]/256, size[1]/2 - subj_text_surface.get_height()/2))
    screen.blit(study_text_surface, ((size[0]+txtsurf_study.get_width())/2-size[0]/64,
...
def display_welcome(image_number):
    screen.fill(BLACK)
    instruction_image = pygame.image.load(
        task_directory + "/media/images/instruction_slides/" + str(image_number) + ".png").convert()
    min_screen_side = min(size)
    instruction_image = pygame.transform.smoothscale(
        instruction_image, (min_screen_side, min_screen_side))
    instruction_rect = instruction_image.get_rect()
    instruction_coords = np.array(
        size)/2 - np.array([instruction_rect.width/2, instruction_rect.height/2])
    screen.blit(instruction_image, instruction_coords)
    display_arrow_key_options()
...
def display_practice_instructions():
    ...
    task_txt = my_font.render(
        "Next, we will practice the task using an example trial.", True, WHITE)
    prompt_txt = my_font.render(
        "(Please select any shape)", True, WHITE)
    screen.blit(task_txt, (size[0]/2 - task_txt.get_width() /
                2, size[1]/2 - task_txt.get_height()/2))
    screen.blit(prompt_txt, (size[0]/2 - prompt_txt.get_width()/2,
                size[1]/2 - prompt_txt.get_height()/2+task_txt.get_height()))
    display_arrow_key_options()
...
def display_movement_warning():
    ...
    my_font = pygame.font.SysFont("Arial", int(size[0]/50))
    task_txt = my_font.render(
        "Try not to talk or move around more than necessary,", True, WHITE)
    prompt_txt = my_font.render(
        "as movements affect the quality of the recordings.", True, WHITE)
    screen.blit(task_txt, (size[0]/2 - task_txt.get_width() /
                2, size[1]/2 - task_txt.get_height()/2))
    screen.blit(prompt_txt, (size[0]/2 - prompt_txt.get_width()/2,
                size[1]/2 - prompt_txt.get_height()/2+task_txt.get_height()))
...
def display_wait_to_start(block_number):
    screen.fill(BLACK)
    txtsurf = FONT.render(
        "Press Right Arrow to Begin Block " + str(block_number), True, WHITE)
    screen.blit(
